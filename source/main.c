...
u32 drawMonsterHP(u32 addr, u32 stride, u32 format)
{
  ...
  if(count > 0)
  {
    ...
    for(u32 i = 0; i < count; i++)
    {
	  ...
	  ovDrawTranspartBlackRect(addr, stride, format, y, x-1, h+2, width+2, 1) ;

      u8 r = 0;
      u8 g = 0;

      if( m->hp > m->hp_max * 0.2) // 20% HP
      {
        g = 255;
      }
      if( m->hp < m->hp_max * 0.3) // 30% HP
      {
        r = 255;
      }

      ovDrawRect(addr, stride, format, y+1, x, h, w, r, g, 0);
    }
    return 0;
  }
  return 1;
